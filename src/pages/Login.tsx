  ...
  const submitForm = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post('/api/v1/auth/login', userToLogin)
      .then((response) => {
        ...
        axios.get(`/api/v1/users/getFollowedUsers?from=${response.data.username}`, {
          headers: {
        ...
        <form className="mt-8 space-y-6" onSubmit={submitForm} method="POST">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-col">
              ...
              <div className="text-sm my-2">
                <Link className="font-medium text-blue-600 hover:text-blue-500" to={"/register"}>Don't have an account?</Link>
              </div>
            </div>
          </div>
